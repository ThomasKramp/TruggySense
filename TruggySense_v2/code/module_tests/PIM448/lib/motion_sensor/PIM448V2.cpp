...
void PIM448::_applyMahonyFilter(float ax, float ay, float az,
    ...
    _integralFBx += TWO_KI * halfex * (1.0f / _sampleFreq);
    _integralFBy += TWO_KI * halfey * (1.0f / _sampleFreq);
    _integralFBz += TWO_KI * halfez * (1.0f / _sampleFreq);
    gx += _integralFBx;
    gy += _integralFBy;
    gz += _integralFBz;
    ...
    _pitch = atan2f(2.0f*(_q0*_q1 + _q2*_q3), 1.0f - 2.0f*(_q1*_q1 + _q2*_q2)) * 360.f / (2 * M_PI);
    _roll  = asinf(2.0f*(_q0*_q2 - _q3*_q1)) * 360.f / (2 * M_PI);
