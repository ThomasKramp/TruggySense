function plotImageOnTrack(imgTrack, positionLatOK, positionLongOK, valueToPlot, colorbarString)
    ...
    if(mean(valueToPlot) ~= 0)
        ...
        % Add index
        set(livePlot, 'ZData', 1:length(coordsOnTrack(:,1)));
        drawnow;
        row1 = dataTipTextRow('X', livePlot.XData);
        row2 = dataTipTextRow('Y', livePlot.YData);
        row3 = dataTipTextRow('Index', livePlot.ZData);
        livePlot.DataTipTemplate.DataTipRows = [row1; row2; row3];
    end
    ...
    hPts = gobjects(4,1);
    for i = 1:4
        hPts(i) = drawpoint('Position', fliplr(originalBox(i,:)), 'Color','r','MarkerSize',10);
    end
