clc;
clear;
imgTrack = imread('track_picture.png');
...
% actMotor_s = [data_slow.ch_2];
% actMotor_s = [data_slow.ch_2] .* ([data_slow.ch_9] > 1050) + (0.5 * [data_slow.ch_9] + 750) .* ([data_slow.ch_9] <= 1050);
actSS_s = [data_slow.ch_4]; %data_slow.pwm_ss

actMotor = [data_fast.pwm_bm_1];
% actMotor = [data_fast.ch_2];
% actMotor = [data_fast.ch_2] .* ([data_fast.ch_9] > 1050) + (0.5 * [data_fast.ch_9] + 750) .* ([data_fast.ch_9] <= 1050);
actSS = [data_fast.ch_4]; %data_fast.pwm_ss
...
legend('PWM_{SS-fast}', 'yaw_{fast}')
% title('Exp2: "Fast Driving", Raw PWM values of SS with yaw angles fast experiment')
...
hold on;
%plot(orientationYaw, LineStyle='-.', LineWidth=2.5);
plot(orientationYaw_s, LineWidth=2.5, LineStyle="--");
hold off;

%legend('PWM_{SS-fast}', 'PWM_{SS-slow}', 'yaw_{fast}', 'yaw_{slow}')
legend('PWM_{SS-slow}', 'yaw_{slow}')
% title('Exp2: "Fast Driving", Raw PWM values of SS with yaw angles slow experiment')
ylabel('Relative Yaw angle (°)');
xlabel('sample point')
